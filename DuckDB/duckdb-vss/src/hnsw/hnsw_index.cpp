...
unique_ptr<IndexScanState> HNSWIndex::InitializeScan(float *query_vector, idx_t limit, ClientContext &context) {
	...
	// Printer::Print("limit" + std::to_string(limit) + "\n");
	// Acquire a shared lock to search the index
	auto lock = rwlock.GetSharedLock();
	USearchIndexType::search_result_t search_result;
	if (result_saved) {
		// Printer::Print("reuse search result\n");
		// search_result = std::move(saved_search_result);
		state->current_row = 0;
		state->total_rows = saved_search_result.size();;
		state->row_ids = make_uniq_array<row_t>(saved_search_result.size());
		result_saved = false;
		saved_search_result.dump_to(state->row_ids.get());
		return std::move(state);
	}
	else {
		// Printer::Print("new search result\n");
...
idx_t HNSWIndex::ScanOnly(float *query_vector, idx_t limit, ClientContext &context) {
	// auto state = make_uniq<HNSWIndexScanState>();
	...
	// Acquire a shared lock to search the index
	auto lock = rwlock.GetSharedLock();
	auto search_result = index.ef_search(query_vector, limit, limit);

	// state->current_row = 0;
	auto total_rows = search_result.size();
	// state->total_rows = search_result.size();
	// state->row_ids = make_uniq_array<row_t>(search_result.size());
...
idx_t HNSWIndex::ScanAndFilter(float *query_vector, idx_t limit, ClientContext &context, float distance_threshold) {
	// Lock and perform the search
	auto lock = rwlock.GetSharedLock();
	// auto search_result = index.ef_search(query_vector, limit, limit);
	saved_search_result = index.ef_search(query_vector, limit, limit);
	
	// saved_search_result = search_result;
	// auto search_result = saved_search_result;
	result_saved = true;

	// Apply the filter and count rows satisfying the condition
	idx_t total_rows = 0;
	distance_threshold = distance_threshold * distance_threshold;
	auto s = saved_search_result.size();
	for (idx_t i = 0; i < s; i++) {
		auto result = saved_search_result[i];
